...
```{r extract programming parameters, echo = FALSE, include=FALSE}
...
```{r plot weekly intensity, eval=F}
...
renderPlotly({
  ...
  temp <- session.weights() %>%
    ...
    group_by(exercise, equipment, variant, weight, sets) %>%
    summarize(
  ...
  p <- temp %>%
    ...
    facet_wrap(~ factor(exercise), scales = "free_y") +
    scale_fill_brewer("Program", palette="Paired") +
