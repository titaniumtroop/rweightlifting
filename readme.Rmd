...
This package comes with a couple of Shiny apps. One lets you see your progress across all programs, and you can filter by date, program, and exercise:
...
```{r Libraries, echo=FALSE, include=TRUE, warning=FALSE, alert=FALSE}
library(ggplot2)
library(plyr)
library(dplyr)
library(data.table)
...
As discussed in the introduction, I wound up switching programs due to this stall. I wanted to monitor my progress given the switch, and see how much stronger I got after switching programs. I looked at this by tonnage and by both max lift.
...
Let's see if that work paid off:
...
Here we see that bench and row showed the most improvement during the Madcow program.
